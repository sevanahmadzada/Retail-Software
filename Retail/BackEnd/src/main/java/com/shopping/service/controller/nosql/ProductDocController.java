...
@RestController
@RequestMapping("/nosql/products")
@RequiredArgsConstructor
public class ProductDocController {

    private final ProductDocService service;
    ...
    @PostMapping
    public ResponseEntity<?> save(@Valid @RequestBody ProductDocument dto){
        return ResponseEntity.ok(service.save(dto));
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable Long id, @Valid @RequestBody ProductDocument dto){
